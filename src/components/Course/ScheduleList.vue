    ...
    confirmEnroll() {
      if (window.isLogged) {
        ...
        .then((response) => {
          ...
          this.$router.push(`/transaction/receipt?sched=${response.data.schedule.schedule_id}&w=${response.data.wallet.id}`);
        }).catch((err) => {
        ...
        this.$router.push(`/login?redirect=/courses/course?id=${this.courses.id}`);
      }
    }
  },
};
